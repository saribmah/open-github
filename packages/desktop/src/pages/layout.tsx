import { Button, Tooltip, DiffChanges, IconButton } from "@open-github/ui";
...
export default function Layout(props: ParentProps) {
  ...
  // Get owner/repo from URL params if present
        ...
        >
          <div class="flex flex-col justify-center items-start gap-4 self-stretch py-2 overflow-hidden mx-auto @[4rem]:mx-0">
            <div class="h-8 shrink-0 flex items-center self-stretch px-3">
              <Tooltip placement="right" value="Collapse sidebar">
                <IconButton
                  ...
                  variant="ghost"
                  size="large"
                  onClick={local.layout.sidebar.toggle}
                />
              </Tooltip>
            </div>
            <div class="w-full px-3">
              <Button
                as={A}
                href={basePath() || "/session"}
                class="hidden @[4rem]:flex w-full"
                size="large"
                icon="edit-small-2"
              >
                New Session
                ...
                <For each={sync.data.session}>
                  {(session) => {
                    const updated = createMemo(() =>
                      DateTime.fromMillis(session.time.updated),
                    );
                    return (
                      <A
                          ...
                          >
                            <div class="flex items-center self-stretch gap-6 justify-between">
                              ...
                              <span class="text-12-regular text-text-weak text-right whitespace-nowrap">
                                {Math.abs(updated().diffNow().as("seconds")) <
                                60
                                  ? "Now"
                                  : updated()
                                      .toRelative({
