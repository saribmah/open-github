...
import { useFilteredList } from "@open-github/ui/hooks";
import {
  createEffect,
  on,
  Component,
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  // Generate base path for navigation (preserve owner/repo if present)
  const basePath = () => {
        ...
        while (currentNode) {
          ...
          if (runningLength + textContent.length >= atMatch.index!) {
            ...
            if (
              ...
              runningLength + textContent.length >= cursorPosition
            ) {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    if (!existing) {
      ...
      if (existing) {
        const sessionPath = basePath()
          ? `${basePath()}/session/${existing.id}`
          : `/session/${existing.id}`;
    ...
    const toAbsolutePath = (path: string) =>
      path.startsWith("/") ? path : sync.absolute(path);
    ...
    <div class="relative size-full _max-h-[320px] flex flex-col gap-3">
      <Show when={store.popoverIsOpen}>
        ...
        >
          <Show
            when={flat().length > 0}
            fallback={<div class="text-text-weak px-2">No matching files</div>}
          >
            <For each={flat()}>
              {(i) => (
                <button
        ...
        <div class="relative p-3 flex items-center justify-between">
          <div class="flex items-center justify-start gap-1">
            ...
            <SelectDialog
              ...
              groupBy={(x) =>
                local.model.recent().includes(x) ? "Recent" : x.provider.name
              }
              sortGroupsBy={(a, b) => {
                const order = [
                  "opencode",
                ...
                if (a.category === "Recent" && b.category !== "Recent")
                  return -1;
                if (b.category === "Recent" && a.category !== "Recent")
                  return 1;
                const aProvider = a.items[0].provider.id;
                const bProvider = b.items[0].provider.id;
                if (order.includes(aProvider) && !order.includes(bProvider))
                  return -1;
                if (!order.includes(aProvider) && order.includes(bProvider))
                  return 1;
              ...
              trigger={
                <Button as="div" variant="ghost">
                  ...
                  <span class="ml-0.5 text-text-weak text-12-regular">
              ...
              {(i) => (
                <div class="w-full flex items-center justify-between gap-x-3">
                  <div class="flex items-center gap-x-2.5 text-text-muted grow min-w-0">
                    <img
                      src={`https://models.dev/logos/${i.provider.id}.svg`}
                      class="size-6 p-0.5 shrink-0 "
                    />
                    <div class="flex gap-x-3 items-baseline flex-[1_0_0]">
                      <span class="text-14-medium text-text-strong overflow-hidden text-ellipsis">
                        {i.name}
                      </span>
                  ...
                  <Show when={!i.cost || i.cost?.input === 0}>
                    <div class="overflow-hidden text-12-medium text-text-strong">
                      Free
                    </div>
                  </Show>
                </div>
